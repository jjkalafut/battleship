...
/**
 * A class for creating Heat Maps as images (for saving to a file or
 * displaying). <p>
 *
 * The
 * <code>HeatChart</code> class describes a chart which can display 3-dimensions
 * of values - x,y and z, where x and y are the usual 2-dimensional axis and z
 * is portrayed by colour intensity. Heat charts are sometimes known as heat
 * maps.
 *
 * <p> Use of this chart would typically involve 3 steps: <ol> <li>Construction
 * of a new instance, providing the necessary z-values.</li> <li>Configure the
 * visual settings.</li> <li>A call to either
 * <code>getChartImage()</code> or
 * <code>saveToFile(String)</code>.</li> </ol>
 *
 * <h3>Instantiation</h3> <p> Construction of a new
 * <code>HeatChart</code> instance is through its one constructor which takes a
 * 2-dimensional array of <tt>doubles</tt> which should contain the z-values for
 * the chart. Consider this array to be the grid of values which will instead be
 * represented as colours in the chart.
 *
 * <p> Setting of the x-values and y-values which are displayed along the
 ...
 * values can be set with the
 * <code>setXValues</code> and
 * <code>setYValues
 * </code> methods. Both methods are overridden with two forms:
 *
 * <h4>Object axis values</h4>
 *
 * <p> The simplest way to set the axis values is to use the methods which take
 * an array of Object[]. This array must have the same length as the number of
 ...
 *
 * <p> This is convenient way of defining numerical values along the axis. One
 * of the two methods takes an interval and an offset for either the x or y
 * axis. These parameters supply the necessary information to describe the
 * values based upon the z-value indexes. The quantity of x-values and y-values
 * is already known from the lengths of the z-values array dimensions. Then the
 ...
 * from the xOffset value 1.0 to 4.0, which is calculated as the number of
 * x-values multiplied by the xInterval, shifted by the xOffset of 1.0. The
 * y-values are calculated in the same way to give a range of values from 0.0 to
 * 6.0.
 *
 * <h3>Configuration</h3> <p> This step is optional. By default the heat chart
 * will be generated without a title or labels on the axis, and the colouring of
 * the heat map will be in grayscale. A large range of configuration options are
 * available to customize the chart. All customizations are available through
 * simple accessor methods. See the javadoc of each of the methods for more
 * information.
 *
 * <h3>Output</h3> <p> The generated heat chart can be obtained in two forms,
 * using the following methods: <ul> <li><strong>getChartImage()</strong> - The
 * chart will be returned as a
 ...
 * application.</li> <li><strong>saveToFile(File)</strong> - The chart will be
 * saved to the file system at the file location specified as a parameter. The
 * image format that the image will be saved in is derived from the extension of
 * the file name.</li> </ul>
 *
...
public class HeatChart {
    ...
    public void setZValues(int[][] zValues) {
        this.zValues = new double[zValues.length][zValues[0].length];

        for (int r = 0; r < zValues.length; r++) {
            for (int c = 0; c < zValues[r].length; c++) {
                this.zValues[r][c] = zValues[r][c];
            }
        }

        this.lowValue = min(this.zValues);
        this.highValue = max(this.zValues);
    }
     ...
     * width="20"><center><strong>y</strong></center></td> <td>1.0</td>
     * <td>1.2</td> <td>1.4</td> </tr> <tr> <td>1.2</td> <td>1.3</td>
     * <td>1.5</td> </tr> <tr> <td>0.9</td> <td>1.3</td> <td>1.2</td> </tr> <tr>
     * <td>0.8</td> <td>1.6</td> <td>1.1</td> </tr> <tr> <td></td> <td
     * colspan="3"><center><strong>x</strong></center></td> </tr> </table>
     *
     * @param zValues the array to replace the current array with. The number of
     * elements in each inner array must be identical.
     * @param low the minimum possible value, which may or may not appear in the
     ...
     * and interval that were given to
     * <code>setXValues(double, double)</code>, in which case the object type of
     ...
     * and interval that were given to
     * <code>setYValues(double, double)</code>, in which case the object type of
     * each element will be
     * <code>Double</code>.
     *
     ...
     *
     * <p> If the title is set to <tt>null</tt> then no title will be displayed.
     *
     * <p> Defaults to null.
     *
     ...
     * the x-axis bar.
     *
     * <p> If the xAxisLabel is set to <tt>null</tt> then no label will be
     * displayed.
     *
     * <p> Defaults to null.
     *
     ...
     * the y-axis bar.
     *
     * <p> If the yAxisLabel is set to <tt>null</tt> then no label will be
     * displayed.
     *
     * <p> Defaults to null.
     *
    ...
    /**
     * Returns the
     * <code>Font</code> that describes the visual style of the title.
     *
    ...
    /**
     * Sets a new
     * <code>Font</code> to be used in rendering the chart's title String.
     *
     * <p> Defaults to Sans-Serif, BOLD, 16 pixels.
     *
    ...
    /**
     * Returns the
     * <code>Color</code> that represents the colour the title text should be
     * painted in.
     *
    ...
    /**
     * Sets the
     * <code>Color</code> that describes the colour to be used for the chart
     * title String.
     *
     * <p> Defaults to
     ...
     *
     * <p> Defaults to Sans-Serif, PLAIN, 12 pixels.
     *
     ...
     * intervals. Both axis use the same font.
     *
     * <p> Defaults to Sans-Serif, PLAIN, 10 pixels.
     *
     ...
     *
     * <p> Defaults to 1. Every column is given a value.
     *
     ...
     * of 3 would mean every third row would be given a value.
     *
     * <p> Defaults to 1. Every row is given a value.
     *
     * @param axisValuesFrequency the frequency of the values displayed against
     ...
     *
     * <p> If axis values are not shown then more space is allocated to the heat
     * map.
     *
     ...
     *
     * <p> The full colour range will go through each RGB step between the high
     ...
     *
     * <p> Defaults to a linear scale value of 1.0.
     *
     ...
     * <code>Image</code> based upon the currently held settings and then
     * attempts to save that image to disk, to the location provided as a File
     * parameter. The image type of the saved file will equal the extension of
     * the filename provided, so it is essential that a suitable extension be
     * included on the file name.
     *
     * <p> All supported
     * <code>ImageIO</code> file types are supported, including PNG, JPG and
     * GIF.
     *
     * <p> No chart will be generated until this or the related
     ...
     * <code>Image</code> configured according to this object's currently held
     * settings. The given parameter determines whether transparency should be
     * enabled for the generated image.
     *
     * <p> No chart will be generated until this or the related
