...
public class CaptainEOF2014 implements Captain {
    ...
    private int 								turnNum;
    private ArrayList<EOF2014AttackType>		attackMethods;
    ...
    public void initialize(int numMatches, int numCaptains, String opponent) {
        ...
        if (!opponent.equals(this.lastOpponent)) {
	         ...
	         * 
	         */
	        this.attackMethods = new ArrayList<EOF2014AttackType>();
            this.attackMethods.add(new EOF2014AttackType(){
        ...
        this.myFleet = randomfleet(); //sp.getNextPlacement(this.wasWin, this.turnNum);
        //this.myFleet = getDistShipPlace();
        this.turnNum = 0;
        
        if( this.match_num == ( numMatches - 1)){
        	System.out.println("Got "+this.ex_cnt+" squares removed");
        }
	...
	private Fleet randomfleet() {
		Fleet fl = new Fleet();
		for( int i = 0; i < 5; i++){
			while(!fl.placeShip(rGen.nextInt(10), rGen.nextInt(10), rGen.nextInt(2), i)){	
			}
		}
		return fl;
	}
        ...
        for (int s = 0; s < 5; s++) {
            ...
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    ...
                    Placement q = new Placement(1, s, new Coordinate(i, j));
                    double scorex = 0;
                    double scorey = 0;
                    boolean ok_x = ( i + shipLen ) < 11;
                    boolean ok_y = ( j + shipLen ) < 11;

                    for (int k = 0; k < shipLen; k++) {
                        scorex += ok_x ? scorer[i + k][j][s] : 0;
                        scorey += ok_y ? scorer[i][j + k][s] : 0;
                    }
                    p.score = scorex;
                    q.score = scorey;
                    if( ok_y ) {places.add(q);}
                    if( ok_x ) {places.add(p);}
                }
            }
            /*
    ...
    public void resultOfAttack(int result) {
        ...
        if (result == MISS || result == DEFEATED) {
        	...
        	for( EOF2014AttackType pat : this.attackMethods){
            ...
            ArrayList<Coordinate> used_coordinates = new ArrayList<Coordinate>();
            ...
            for( Placement p: places){
            	...
            	for( int i = 0; i< used_coordinates.size(); i++){
            		Coordinate c = used_coordinates.get(i);
            		if(!p.contains(c)){
            			used_coordinates.remove(i);
            			i--;
            		}
            	}
            }
            for( int j = 0; j< used_coordinates.size(); j++){
        		Coordinate c = used_coordinates.get(j);
        		if( this.myMatchShots[c.getX()][c.getY()]){
        			used_coordinates.remove(j);
        			j--;
        		}        		
            }
            for( Coordinate c: used_coordinates ){
            	this.ex_cnt++;
	            for (int s = 0; s < 5; s++) {
	                if (this.shipsAlive[s] && s != shipMod) {
	                    ArrayList<Placement> tmp_places = this.theirPlacements.get(s);
	                    ArrayList<Placement> bads = new ArrayList<Placement>();
	                    for (Placement p : tmp_places) {
	                        if (p.contains(c)) {
	                            bads.add(p);
	                        }
	                    }
	                    tmp_places.removeAll(bads);
	                } 
	            }
            }
        }
	...
	*/
	private abstract class EOF2014AttackType{
