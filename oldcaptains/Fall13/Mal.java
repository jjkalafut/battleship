...
public class Mal implements Captain {
    ...
    public void initialize(int numMatches, int numCaptains, String opponent) {
        ...
        smallestShip = 4;
        largestShipModel = AIRCRAFT_CARRIER;
    ...
    void placePatrolBoat() {
        ArrayList<int[]> bestSquares = new ArrayList<>();
        double minValue = 9999999, horiz, vert;
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 10; j++) {
                horiz = currentOpponentAP[i][j] + currentOpponentAP[i + 1][j];
                vert = currentOpponentAP[j][i] + currentOpponentAP[j][i + 1];
                if (horiz == minValue) {
                    bestSquares.add(new int[]{i, j, HORIZONTAL});
                } else if (horiz < minValue) {
                    bestSquares = new ArrayList<>();
                    bestSquares.add(new int[]{i, j, HORIZONTAL});
                    minValue = horiz;
                }
                if (vert == minValue) {
                    bestSquares.add(new int[]{j, i, VERTICAL});
                } else if (vert < minValue) {
                    bestSquares = new ArrayList<>();
                    bestSquares.add(new int[]{j, i, VERTICAL});
                    minValue = vert;
                }
        ...
        int[] bestPosition = bestSquares.get(generator.nextInt(bestSquares.size()));
        myFleet.placeShip(bestPosition[0], bestPosition[1], bestPosition[2], PATROL_BOAT);
    }
    ...
    public Coordinate makeAttack() {
        ...
        if (!hitShips) {
            double maxValue = 0;
            double thisValue;
            ArrayList<square> bestSquares = new ArrayList<>();
            square bestSquare;
            if (queue.isEmpty()) {
                makeQueue();
            }
            for (square s : queue) {
                thisValue = s.getValue(shipLengths[largestShipModel]) * currentOpponent.shipProb(largestShipModel, s.x, s.y);
    ...
    public void resultOfAttack(int result) {
        ...
        if (result == MISS) {
            currentOpponent.misses[lastX][lastY]++;
        }
        for (int i = 1; i < 10; i++) {
    ...
    public void resultOfGame(int result) {
        currentOpponent.rounds++;
        if (currentOpponent.rounds % 100 == 0) {
            currentOpponent.attackPattern = new double[10][10];
        }
    }
    ...
    Coordinate continueAttack() {
        ...
        if (hCounter > 0) {
            if ((board[hitX + hCounter][hitY].right >= 1) && (board[hitX][hitY].left >= 1)) {
                return (currentOpponent.shipProb(ship, hitX + hCounter + 1, hitY) > currentOpponent.shipProb(ship, hitX - 1, hitY))
                        ? new Coordinate(hitX + hCounter + 1, hitY) : new Coordinate(hitX - 1, hitY);
            } else if (board[hitX + hCounter][hitY].right >= 1) {
                return new Coordinate(hitX + hCounter + 1, hitY);
            } else {
                return new Coordinate(hitX - 1, hitY);
            }
        } else if (vCounter > 0) {
            if ((board[hitX][hitY + vCounter].up >= 1) && (board[hitX][hitY].down >= 1)) {
                return (currentOpponent.shipProb(ship, hitX, hitY + vCounter + 1) > currentOpponent.shipProb(ship, hitX, hitY - 1))
                        ? new Coordinate(hitX, hitY + vCounter + 1) : new Coordinate(hitX, hitY - 1);
            } else if (board[hitX][hitY + vCounter].up >= 1) {
    ...
    public void makeQueue() {
        ...
        queue = new LinkedList<>();
        smallestShip = AIRCRAFT_CARRIER_LENGTH;
        largestShipModel = PATROL_BOAT;
        for (Integer i : remainingShips.keySet()) {
            if (remainingShips.get(i) < smallestShip) {
                smallestShip = remainingShips.get(i);
            }
            if (remainingShips.get(i) > shipLengths[largestShipModel]) {
                largestShipModel = i;
            }
        }
        for (int i = 0; i < 10; i++) {
    ...
    class square {
        ...
        public int getValue(int shipLength) {
            int value = 0;
    ...
    class opponentInfo {
        ...
        double[][][] shipPattern;
        double[][] attackPattern;
        double[][] misses;
        int rounds;

        opponentInfo() {
            attackPattern = new double[10][10];
            shipPattern = new double[5][10][10];
            misses = new double[10][10];
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    for (int model = 0; model < 5; model++) {
                        shipPattern[model][i][j]++;
                    }
                }
            }
        }

        double shipProb(int ship, int x, int y) {
            return shipPattern[ship][x][y] / (misses[x][y] + shipPattern[ship][x][y]);
        }
    }
}