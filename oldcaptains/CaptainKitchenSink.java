 ...
 * tries to favor the hits and avoid the misses later on. This is very effective
 * at exploiting any AIs that have a 'pattern' to their ship placement. He
 * places his ship according to one of 5 different strategies. Initially all 5
 * are used for a while to see which one is most effective and then the best
 * defensive strategy is chosen and used for the remainder of matches with that
 * opponent. He literally throws every defensive strategy at the opponent
 * (including the 'kitchen sink') to see what works.
 */
public class CaptainKitchenSink implements Captain {
    ...
    protected HashMap< String, ArrayList<PriorityCoordinate>> mSearchPattern;
    ...
    protected static final int DEFENSE_COUNT = 5;
    // Knowledge of current layout
    protected boolean[] mAttempted;

    public CaptainKitchenSink() {
        mSearchPattern = new HashMap<String, ArrayList<PriorityCoordinate>>();
        ...
        mBackupSearchPattern = new ArrayList<Coordinate>(50);
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 5; j++) {
                if (i % 2 == 0) {
                    mBackupSearchPattern.add(new PriorityCoordinate(i, j * 2));
                } else {
                    mBackupSearchPattern.add(new PriorityCoordinate(i, j * 2 + 1));
                }
            }
        }
    ...
    public void initialize(int pNumMatches, int pNumCaptains, String pOpponent) {
        ...
        OFFENSIVE_WARMUP_MATCHES = Math.round(0.25 * pNumMatches);
        DEFENSIVE_WARMUP_MATCHES = Math.round(0.5 * pNumMatches);

        // Add a search list for this opponent if one doesn't already exist
        if (!mSearchPattern.containsKey(pOpponent)) {
            ArrayList<PriorityCoordinate> newList = new ArrayList<PriorityCoordinate>(50);
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 5; j++) {
                    if (i % 2 == 0) {
            ...
            mSearchPattern.put(pOpponent, newList);
        ...
        // While we're warming up, randomize our search for equal sampling
        if (mMatchCount.get(mCurEnemy) < OFFENSIVE_WARMUP_MATCHES) {
            Collections.shuffle(mSearchPattern.get(mCurEnemy));
        }
        ...
        if (mCurDefense == -1) {
            mCurDefense = mMatchCount.get(mCurEnemy) % 3;
        }

        // Pick a defensive strategy
        switch (mCurDefense) {
            case 0:
                positionShipsAtEdge();
                break;
            ...
            case 4:
                ...
                break;
//        	default: positionShipsClustered(); break;
        }
    }
    ...
    protected void positionShipsClustered() {
        ...
        for (int shipType = 4; shipType >= 0; shipType--) {
            ...
            do {
                ...
                // Ensure ship is inside the 5x5 square
                badSpot = false;
    ...
    public Coordinate makeAttack() {
        ...
        // First, check if we've found any that are not yet sunk
        for (int i = 0; i < 5; i++) {
        ...
        ArrayList<PriorityCoordinate> lCurList = mSearchPattern.get(mCurEnemy);
        while (lCoord == null) {

            Coordinate lCandidate = null;
            if (mSearchSpot < 50) {
                while (mSearchSpot < lCurList.size() && !isValidNextAttack(lCandidate)) {
                    lCandidate = lCurList.get(mSearchSpot);
                    mSearchSpot++;
                }
            } else if (mSearchSpot >= 50 && mSearchSpot < 100) {
                while (mSearchSpot - 50 < mBackupSearchPattern.size() && !isValidNextAttack(lCandidate)) {
                    lCandidate = mBackupSearchPattern.get(mSearchSpot - 50);
                    mSearchSpot++;
                }
            } else {
...
//					defString, mCurEnemy, mMatchCount.get(mCurEnemy));
    ...
    public void resultOfAttack(int result) {
        ...
        if (isHit(result)) {
            ...
            if (mMatchCount.get(mCurEnemy) < OFFENSIVE_WARMUP_MATCHES) {
                ...
                int index = mSearchPattern.get(mCurEnemy).indexOf(mLastAttack);
                if (index != -1) {
                    mSearchPattern.get(mCurEnemy).get(index).hits++;
                }
            }
            ...
            if (mMatchCount.get(mCurEnemy) < OFFENSIVE_WARMUP_MATCHES) {
                ...
                int index = mSearchPattern.get(mCurEnemy).indexOf(mLastAttack);
                if (index != -1) {
                    mSearchPattern.get(mCurEnemy).get(index).misses++;
                }
            }
        }
    }
    ...
    public void resultOfGame(int result) {
        ...
        // Is it time to sort our search pattern?
        if (mMatchCount.get(mCurEnemy) == OFFENSIVE_WARMUP_MATCHES) {
            // Update all the priorities
            ArrayList<PriorityCoordinate> myList = mSearchPattern.get(mCurEnemy);
            for (int i = 0; i < myList.size(); i++) {
                myList.get(i).updatePriority();
            }
    ...
    private class PriorityCoordinate extends Coordinate implements Comparable<PriorityCoordinate> {
        ...
        public int compareTo(PriorityCoordinate o) {
            Long me = new Long(priority);
            return me.compareTo(o.priority);
        }
    }
